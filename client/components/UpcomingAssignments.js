...
export function mountUpcomingAssignments({ containerId = "upcomingAssignmentsContainer" } = {}) {
  ...
  function renderAssignmentBox(assignment) {
    ...
    box.innerHTML = `
      ...
      <div class="flex-1 min-w-0">
        ...
        <!-- Assignment title -->
        <h3 class="text-base font-semibold text-slate-900 truncate mb-1">
          ${assignment.title}
        </h3>
    ...
    // Add click handler if needed (could open assignment details)
    ...
    if (upcomingAssignments.length === 0) {
      container.innerHTML = `
        <div class="rounded-xl border border-dashed border-slate-300 bg-slate-50 p-8 text-center">
          ...
          <h3 class="text-base font-medium text-slate-700 mb-1">All Caught Up!</h3>
          <p class="text-sm text-slate-500">No upcoming assignments at the moment.</p>
        </div>
      `;
      return;
    }
  ...
  async function reload() {
    try {
      ...
      container.innerHTML = `
        <div class="rounded-xl border border-dashed border-red-300 bg-red-50 p-4 text-sm text-red-600">
          ${e.message}
        </div>
      `;
    }
